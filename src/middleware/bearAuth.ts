...
export const verifyToken = (
  ...
  } catch (err: unknown) {  // Explicitly specify `unknown` type
...
const authMiddlewareFactory = (allowedRoles: UserRole | UserRole[]) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (!allowed.includes(decoded.role)) {
      console.warn(`[authMiddleware] Access denied for role '${decoded.role}'. Allowed: ${allowed.join(', ')}`);
...
export const refreshTokenMiddleware = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!process.env.JWT_REFRESH_SECRET) {
    res.status(500).json({ error: 'Server misconfiguration: JWT_REFRESH_SECRET missing' });
    return;
  }
  ...
  // If the refresh token is valid, attach the user to the request
  req.user = decoded;
  next();
};
...
export const verifyTokenForRefresh = refreshTokenMiddleware;
